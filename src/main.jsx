...
function GuidedTour({ onFinish, onSectionChange, onSettingsChange }) {
  const steps=[
    {section:'Hoje',target:'[data-tour="welcome"]',title:'Seu dia começa aqui',text:'O painel Hoje reúne tarefas, progresso e o que merece sua atenção agora.'},
    {section:'Hoje',target:'[data-tour="new-task"]',title:'Crie sua próxima tarefa',text:'Informe o conteúdo, a matéria, a data e o tempo previsto. Comece pequeno e avance no seu ritmo.'},
    {section:'Hoje',target:'[data-tour="focus"]',title:'Transforme planejamento em foco',text:'Escolha uma matéria e estude por 25 ou 50 minutos. Cada sessão concluída alimenta suas metas.'},
    {section:'Planejamento',target:'[data-tour="planning"]',title:'Planejamento completo',text:'Aqui você reúne todas as tarefas, filtra pendências e conclui o que já foi estudado.'},
    {section:'Planejamento',target:'[data-tour="exams"]',title:'Provas também ficam organizadas',text:'Cadastre a avaliação, matéria, data e, se quiser, o nome do professor. Assim nada chega de surpresa.'},
    {section:'Calendário',target:'[data-tour="calendar"]',title:'Veja sua rotina no calendário',text:'Tarefas e provas aparecem nas datas corretas para você enxergar semanas mais carregadas.'},
    {section:'Matérias',target:'[data-tour="subjects-page"]',title:'Suas matérias em um só lugar',text:'Adicione, edite ou remova disciplinas e acompanhe o avanço semanal de cada uma.'},
    {section:'Metas',target:'[data-tour="goals"]',title:'Metas possíveis de cumprir',text:'Defina quantas sessões deseja fazer por matéria e acompanhe sua constância sem pressão excessiva.'},
    {section:'Metas',settings:true,target:'[data-tour="appearance"]',title:'Deixe o aplicativo com a sua cara',text:'Nas configurações você escolhe modo claro ou escuro, cor principal, lembretes, segurança e backup.'},
    {section:'Hoje',target:()=>window.innerWidth<=1000?'[data-tour="menu"]':'[data-tour="navigation"]',title:'Pronto para começar',text:'Use este menu para circular pela plataforma. Você pode refazer esta apresentação quando quiser nas configurações.'}
  ]
  const [step,setStep]=useState(0)
  ...
  useEffect(()=>{
    ...
    onSectionChange(current.section)
    onSettingsChange(Boolean(current.settings))
    setBox(null)
    const update=()=>{
      const selector=typeof current.target==='function'?current.target():current.target
    ...
    timer=setTimeout(update,current.settings?350:250)
  ...
  const tooltipStyle=box?{left:Math.max(16,Math.min(box.left,window.innerWidth-336)),top:box.top+box.height+220<window.innerHeight?box.top+box.height+16:Math.max(16,box.top-215)}:{left:'50%',top:'50%',transform:'translate(-50%,-50%)'}
...
function WorkspacePage({ active, tasks, subjects, exams, sessions, setTasks, setSubjects, setExams, openTask }) {
  ...
  if (active === 'Planejamento') return <>
    <div className="welcome" data-tour="planning"><div><h1>Planejamento</h1><p>Organize o que precisa ser feito, sem sobrecarregar seu dia.</p></div><button className="primary" onClick={openTask}><Plus size={19}/> Nova tarefa</button></div>
    <section className="exam-section" data-tour="exams">
  ...
  return <>
    ...
    <div className="goal-hero" data-tour="goals"><div><span>PROGRESSO DA SEMANA</span><strong>{weeklyCompleted} de {weeklyGoal} sessões concluídas</strong><p>{weeklyPct>=100?'Meta semanal alcançada!':'Continue no seu ritmo — cada sessão conta.'}</p></div><div className="goal-circle">{weeklyPct}%</div></div>
...
function App() {
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content">Ir para o conteúdo</a>
    ...
    {tourOpen&&<GuidedTour onSectionChange={section=>{setActive(section);setMobileNav(false)}} onSettingsChange={setSettingsOpen} onFinish={()=>{setTourOpen(false);setSettingsOpen(false);setActive('Hoje');setSettings(current=>({...current,tourComplete:true}))}}/>}
  </div>
}
